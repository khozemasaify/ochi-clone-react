...
function Navbar() {
  return (
    <div className=" z-[999] fixed font-neue-montreal w-full px-16 py-6 flex  items-center justify-between ">
      <motion.div initial = {{scale:0}} animate={{scale:1}}    transition={{ease:[0.45, 0, 0.55, 1],delay:.2, duration:1}} className="logo">
        <svg
          width="72"
          height="30"
          viewBox="0 0 72 30"
      ...
      <div  className=" font-['Neue Montreal'] overflow-hidden midnav flex items-center ml-72 gap-[2vw] ">
        {["Services","Our work","About us","Insights"].map((item,index) => (
          <motion.a initial = {{y:100}} animate={{y:0}}    transition={{ease:[0.45, 0, 0.55, 1],delay:index*.2, duration:1}} className=" text-[18px]     font-light " key={index}>{item}</motion.a>
        ))}
      </div>
      <div className="lastnav overflow-hidden">
        <motion.h3 initial = {{y:100}} animate={{y:0}}    transition={{ease:[0.45, 0, 0.55, 1], duration:1}} >Contact us</motion.h3>
      </div>
    </div>
  );
}
